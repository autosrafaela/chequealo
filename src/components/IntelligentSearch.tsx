      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSearch(searchQuery); }}>
        <div className="relative">
          <Input
            ...
            className="h-12 sm:h-14 md:h-16 text-sm sm:text-base md:text-lg px-4 sm:px-5 md:px-6 pr-24 sm:pr-28 bg-background/95 border-0 rounded-xl sm:rounded-2xl placeholder:text-muted-foreground/60 focus-visible:ring-primary/30 shadow-sm"
          />
          
          <div className="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-1 sm:gap-1.5">
            {/* Voice search button */}
            <Button
              type="button"
              variant="ghost"
              ...
              className={`h-8 w-8 sm:h-9 sm:w-9 p-0 rounded-full ${isRecording ? 'text-destructive animate-pulse' : 'text-muted-foreground hover:text-primary'}`}
            >
            ...
            <Button 
              ...
              size="sm"
              disabled={aiLoading}
              className="bg-primary hover:bg-primary/90 h-8 sm:h-10 px-3 sm:px-4 rounded-xl"
            >
              {aiLoading ? (
